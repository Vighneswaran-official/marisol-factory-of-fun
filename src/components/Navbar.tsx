...
import { 
  Flame, Volume2, ArrowLeft, Menu, X, User, Download
} from 'lucide-react';
      ...
      <header className="sticky top-0 z-40 bg-[#FAF7F0]/95 backdrop-blur-md border-b-2.5 border-ink px-3 sm:px-6 py-2.5 shadow-paper">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5">
            ...
            {/* Google Sign In / Profile Avatar (Desktop quick pill) */}
            {onOpenGoogleSignIn && (
              <button
                onClick={() => {
                  audioEngine.playSfx('click');
                ...
                className={`hidden md:flex p-1.5 px-2.5 rounded-xl border-2 border-ink shadow-sketch transition-all items-center gap-1.5 ${
                  isAuthenticated ? 'bg-white hover:bg-emerald-50' : 'bg-white hover:bg-purple-50'
                }`}
                title={isAuthenticated && currentUser ? `Signed in as ${currentUser.name}` : "Sign in / Sync Email"}
              >
            ...
            <div>
              ...
              <div className="mt-3 bg-gradient-to-r from-purple-50 to-pink-50 border-2 border-purple-200 rounded-2xl p-3 flex items-center justify-between shadow-2xs">
                ...
                <button
                  ...
                  className="sketch-btn px-2 py-1 text-[11px] font-black uppercase bg-white border border-purple-300 shadow-2xs shrink-0"
                >
