...
export const BatchUpdatesWall: React.FC<BatchUpdatesWallProps> = ({ onNavigate }) => {
  ...
  // New Post Form State
  const [studentName, setStudentName] = useState(player.nickname || 'Kritika');
  const [selectedPose, setSelectedPose] = useState(player.activeSticker || 'brighter_ideas');
  const [selectedMood, setSelectedMood] = useState('Radiant & Grateful');
  ...
  useEffect(() => {
    const unsub = batchWallService.subscribe(() => setTick(t => t + 1));
    return () => { unsub(); };
  }, []);
  ...
  const handleCreatePost = (e: React.FormEvent) => {
    ...
    batchWallService.addPost({
      studentName: studentName.trim() || 'Kritika',
    ...
    <div className="min-h-screen bg-[#FFFDF7] p-3 sm:p-6 pb-28 text-ink">
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 bg-white border-2 border-ink px-2.5 py-1 rounded-full text-xs font-handwritten font-bold shadow-sketch">
            {network.isOnline ? (
              ...
              <span className="text-amber-600 flex items-center gap-1">
                <WifiOff className="w-3.5 h-3.5" /> Queue ({network.queuedCount})
              </span>
            )}
          </div>
        </div>

        {/* Hero Card & New Post Trigger */}
        <div className="bg-gradient-to-r from-purple-50 via-pink-50 to-amber-50 border-3 border-ink rounded-3xl p-4 sm:p-5 shadow-sketch-lg flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="space-y-1 text-center sm:text-left">
            <div className="font-display font-black text-lg text-ink">
              Batch MLP41PT Memories & Live Updates
            </div>
            <p className="font-handwritten text-xs sm:text-sm text-ink-light font-bold">
              Post notes, memories, cheer each other on, and react with our 11 custom stickers!
            </p>
          </div>
          ...
          <button
            ...
            className="sketch-btn-primary px-4 py-2.5 text-xs sm:text-sm font-black uppercase flex items-center gap-1.5 shadow-sketch shrink-0"
          >
